...
with st.sidebar:
    ...
    remember_key_toggle = st.checkbox("Remember API Key", value=bool(st.session_state.api_key))
    ...
    persona_selection = st.selectbox(
        "Select Persona",
        ["General Assistant", "Python Expert", "Creative Writer", "Data Analyst", "Code Reviewer"]
    )
    st.divider()
...
if prompt := st.chat_input("Ask Aura anything..."):
    ...
    with chat_container:
        ...
        with st.chat_message("assistant"):
            ...
            for partial_text in get_gemini_stream(prompt, persona_selection, active_document):
